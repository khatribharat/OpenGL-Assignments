...
void  DrawGLScene(void)
{
   ...
   //#############################################  CREATING THE OCT-TREES ( ONLY ONCE ) ##########################################################
   if ( octTreeAvailable[0] == 'N' )
   	{ 
	   
	  objTree[0] = voxelSpacePartition(&cylinderFunc,0.0,0.0,0.0,5.0);
	  octTreeAvailable[0] = 'Y';
	}
   
   if ( octTreeAvailable[1] == 'N' )
   	{ 
	   
	  objTree[1] = voxelSpacePartition(&sphereFunc,0.0,0.0,0.0,5.0);
	  octTreeAvailable[0] = 'Y';
	}

   //###########################################################################################################################################

   //##################################################  RENDERING THE OCT-TREES ###############################################################
   ...
   octTreeTraversal(objTree[0]);
    
   glTranslatef(5.0f, -2.0f, -4.0f);
   glMaterialfv( GL_FRONT, GL_AMBIENT_AND_DIFFUSE, giveColorArray (0.5,0.5,0.5,1.0) );
   glMaterialfv( GL_BACK , GL_AMBIENT_AND_DIFFUSE, giveColorArray (0.5,0.5,0.5,1.0) ); 
   octTreeTraversal(objTree[1]);
